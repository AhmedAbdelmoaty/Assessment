(function () {
    ...
    (async function init() {
      ...
      const guardResult = await authGuardAndLoad();
      ...
      // لو ما عندناش رسائل (أو ما وصلش sessionId) نبدأ الـ intake لضمان ظهور البداية
      const hasMessages = (guardResult.loadedMessages || 0) > 0;
      if (!hasMessages) {
        startIntakeFlow();
      }
    })();
