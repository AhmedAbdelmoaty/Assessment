(function () {
  ...
  const selectors = {
    ...
    averageScoreFill: document.getElementById("averageScoreFill"),
    ...
    list.forEach((item) => {
      ...
      card.innerHTML = `
        ...
        <div class="progress-bar">
          <div class="progress-fill" data-target="${percent}"></div>
        </div>
  ...
  function updateAssessmentSummary() {
    ...
    const avg = total ? Math.round(state.assessments.reduce((sum, a) => sum + (a.percent || 0), 0) / total) : 0;
    if (selectors.averageScore) selectors.averageScore.textContent = `${avg}%`;
    if (selectors.assessmentsCount) selectors.assessmentsCount.textContent = total;
    if (selectors.averageScoreFill) {
      selectors.averageScoreFill.style.width = "0%";
      requestAnimationFrame(() => {
        selectors.averageScoreFill.style.width = `${avg}%`;
      });
    }
    selectors.heroAssessments.textContent = total;
  }
