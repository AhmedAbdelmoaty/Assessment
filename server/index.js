...
async function ensureOpeningMessages(chatSession, state, lang = "en") {
  if (!chatSession?.id || !state) return false;

  // نضمن وجود لغة محددة
  state.lang = state.lang || lang || "en";

  let changed = false;

  // لو لسه ماعرضناش الافتتاحية نضيفها
  if (!state.openingShown && state.currentStep === "intake") {
    await persistMessage(chatSession, state, "assistant", INTAKE_OPENING[state.lang] || INTAKE_OPENING.en);
    state.openingShown = true;
    changed = true;
  }

  // لو ما فيش أي رسائل (أو فقط الافتتاحية) نضيف أول سؤال من الـ intake
  const msgsCount = Array.isArray(state.messages) ? state.messages.length : 0;
  const needsFirstPrompt = state.currentStep === "intake" && (msgsCount === 0 || (msgsCount === 1 && state.openingShown));

  if (needsFirstPrompt) {
    const stepKey = INTAKE_ORDER[state.intakeStepIndex || 0];
    const step = INTAKE_CATALOG[stepKey];
    if (step?.prompt?.[state.lang]) {
      await persistMessage(chatSession, state, "assistant", step.prompt[state.lang]);
      changed = true;
    }
  }

  if (changed) {
    await saveSessionState(chatSession.id, state);
  }
...
app.get("/api/chat/current", requireAuth, async (req, res) => {
  ...
  // لغة المستخدم المفضلة (إن وجدت)
  let userLang = state.lang || "en";
  try {
    const u = await pool.query("SELECT locale FROM users WHERE id=$1", [uid]);
    if (u.rows[0]?.locale) {
      userLang = u.rows[0].locale;
      state.lang = userLang;
    }
  } catch {}

  // لو الجلسة لسه بلا رسائل، نجهز رسالة الترحيب وأول سؤال
  try {
    await ensureOpeningMessages(chatSession, state, userLang);
  } catch (e) {
    console.error("ensureOpeningMessages failed", e);
  }
